...
`streamlit-app` is a thin manual integration client. It talks only to `conversation-service`; it has no direct DB, MCP, Milvus, or model access.
...
It can create or reload persisted conversations, display persisted messages, stream agent events over SSE, and inspect plans/tool calls/fallbacks for end-to-end testing.
